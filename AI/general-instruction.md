...
# 用語・語彙
* 造語や雰囲気語を使うな。
* 定訳が不明な語を和訳するな。その場合標準表記(英語/カタカナ)のまま使う。
  * 例: time series data などの window は windowやウィンドウと呼ぶべき。"窓" 等の日本語として使わない直訳を使うな
* 日本語に直訳しした場合に不自然な表現を使うな。
  * 悪い例: "XXXイベントが発生すると、YYY動作がgate される"
