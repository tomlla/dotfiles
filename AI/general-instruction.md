...
# 私からの指示について
* 私からの質問は修正タスクやタスクの実行を依頼しているのではなく、なんらかの回答を求めている.
  * 質問の時はタスク実行や修正をせずに質問に回答して。

# あなたの回答形式の指定

- 私は英語で質問を投げることもありますが、日本語で回答してください。
- LLMの回答文章内で、fileのpath:line を表示すると見にくいので、行を分けて.
- DO NOT USE TABLE STYLE ANSWER!
...
- Naming
  - "Manager"というsuffixは避ける。責務が明確でないため.
  - 以下の名前は避ける
    * `check`, `data`
